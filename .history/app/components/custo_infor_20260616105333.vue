        ...
        <div :class="section.wrapperClass" class="">
          <template v-for="(item, index) in section.options" :key="item.id">
            <label class="flex items-center text-bace text-[#6C757D] cursor-pointer">
              <input :type="section.type" :name="section.id" :value="item.id" v-model="formModels[section.id]" class="appearance-none w-5 h-5 rounded-full border border-[#CED4DA] bg-white checked:border-primary checked:bg-primary mr-2 max-sm:w-4 max-sm:h-4">
              {{ item.label }}
            </label>
          </template>
        </div>
