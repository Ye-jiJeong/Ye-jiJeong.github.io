...
const inputFields = ref([
  ...
  },
  {
    ...
    btnText: '인증완료',
    btnClass: 'w-24 flex items-center justify-start text-primary font-bold whitespace-nowrap gap-1.5',
    isText: true,
  ...
  <div class="w-full flex flex-col items-center">
    ...
    <div class="w-full max-w-128 bg-muted-light border border-bs-gray rounded-2xl p-6 flex flex-col gap-2 max-sm:gap-4">
      <div v-for="field in inputFields" :key="field.id" class="flex gap-2 max-sm:flex-col">
        <input :id="field.id" :type="field.type" :placeholder="field.placeholder" :disabled="field.isText" autocomplete="username" aria-required="true" class="flex-1 h-12 px-4 bg-white border border-gray-300 rounded-lg text-base focus:outline-none focus:border-primary disabled:bg-slate-100 disabled:text-slate-500 max-sm:py-3.75">
        ...
        <button v-else type="button" :class="field.btnClass" class="max-sm:w-full">
          <i v-if="field.iconName" class="text-lg" :class="field.iconName"></i>{{ field.btnText }}
        </button>
      </div>
