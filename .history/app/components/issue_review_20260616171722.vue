...
<template>
  <div class="w-full">
    ...
    <!-- 교정: max-sm 구간에서 컨테이너 전체 너비를 확보하여 하위 요소들이 완전한 좌우 정렬을 갖추도록 제어 -->
    <div class="mt-13.5 flex items-center justify-between border-slate-100 mb-4 max-md:mt-8 max-sm:flex-col-reverse max-sm:gap-3 max-sm:w-full">
      <!-- 교정: max-sm 구간에서 grid 구성을 추가하여 3개의 셀렉트 박스가 화면 가로폭을 3등분으로 꽉 채우도록 구현 -->
      <div class="flex gap-2 mb-7.5 max-sm:mb-0 max-sm:w-full max-sm:grid max-sm:grid-cols-3 max-sm:gap-2">
        <div v-for="category in filterCategories" :key="category.id" class="relative max-sm:w-full">
          <select 
            v-model="category.selected" 
            :class="['appearance-none border border-slate-200 rounded-lg py-2 px-4 max-md:px-3 text-base max-md:text-sm text-slate-700 cursor-pointer focus:outline-none focus:border-blue-500 max-sm:w-full max-sm:px-2 max-sm:text-xs', category.widthClass]"
          >
            <option value="">{{ category.defaultText }}</option>
            <option v-for="(opt, idx) in category.options" :key="idx" :value="opt">{{ opt }}</option>
          </select>
          
          <icon name="bxs:down-arrow" class="absolute right-4.5 max-md:right-3 max-sm:right-2.5 top-1/2 -translate-y-1/2 text-xs max-md:scale-75 text-slate-500" />
        </div>
      </div>

      <div class="flex items-center text-base max-md:text-sm mb-7.5 max-sm:mb-0 max-sm:ml-auto">
    ...
    <div class="flex flex-col gap-7.5 max-md:gap-4">
      ...
      >
        <img :src="reviewImages[index % 4]" alt="고객 프로필" class="flex-shrink-0 max-md:w-12 max-md:h-12 max-sm:self-stretch max-sm:w-auto max-sm:h-auto max-sm:object-cover" />
        ...
        <p class="w-full text-base mt-1 leading-relaxed text-slate-900 sm:hidden">{{ review.content }}</p>
      </div>
    </div>
