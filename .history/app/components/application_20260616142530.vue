...
<template>
  <section class="flex items-end justify-between gap-6 max-md:!items-start">
    ...
    <div class="w-fit flex-shrink-0 max-lg:w-[46%] self-start">
      <div class="relative inline-block w-full whitespace-nowrap">
        <img :src="imgUrl" alt="비주얼 이미지" class="w-full" />
        <p v-if="visualBadgeText" class="absolute top-6 right-5 inline-block rounded-lg bg-warning py-1 px-4 font-bold text-lg max-md:rounded max-md:top-3 max-md:right-3 max-md:py-0.5 max-md:px-2 max-md:text-base">
            {{ visualBadgeText }}
        </p>
      </div>
    </div>
