...
<template>
  <section class="flex items-end justify-between gap-6 max-md:!items-start max-sm:flex-col-reverse max-sm:gap-5">
    <div class="flex-1 min-w-0 max-sm:w-full">
      ...
      <h1 class="text-5xl font-bold mt-6.75 mb-5 leading-snug max-md:text-4xl max-sm:text-3xl max-sm:mt-5 max-sm:mb-4">
      ...
      <ul class="flex flex-col gap-1">
          <li v-for="(item, idx) in checkList" :key="idx" class="text-xl flex items-center gap-2.5 font-medium whitespace-nowrap max-md:text-base max-sm:whitespace-normal max-sm:items-start max-sm:gap-2">
            ...
            <span class="max-sm:inline-block max-sm:align-top">{{ item.text || item }}</span>
            <Button v-if="item.btnText" :text="item.btnText" class="!text-base text-primary !p-0 hover:[&>span]:underline max-sm:inline-block max-sm:ml-1" iconClass="translate-y-[-1px]" />
          </li>
      </ul>
    </div>

    <div class="w-fit flex-shrink-0 max-lg:w-[46%] self-start max-sm:w-full">
      <div class="relative inline-block w-full">
        <img :src="imgUrl" alt="비주얼 이미지" class="w-full rounded-2xl" />
        <p v-if="visualBadgeText" class="absolute top-6 right-5 inline-block rounded-lg bg-warning py-1 px-4 font-bold text-lg whitespace-nowrap max-md:rounded max-md:top-3 max-md:right-3 max-md:py-0.5 max-md:px-2 max-md:text-base max-sm:text-sm max-sm:top-2.5 max-sm:right-2.5">
            {{ visualBadgeText }}
        </p>
      </div>
    </div>
