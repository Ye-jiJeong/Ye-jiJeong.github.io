  ...
  <div class="w-full">
    ...
    <div class="hidden max-md:block w-full bg-[#ebf2f4]/60 p-6 rounded-3xl text-left">
      ...
      <div class="flex flex-col gap-3 w-full">
        <div v-for="cert in (isExpanded ? certList : certList.slice(0, 6))" :key="'mo-' + cert.id" class="w-full bg-white rounded-xl py-4 px-4 flex items-center justify-center text-center shadow-sm border border-slate-100/50">
          <span class="text-base font-medium text-slate-800 tracking-tight">{{ cert.name }}</span>
        </div>

        <button @click="isExpanded = !isExpanded" class="w-full border border-blue-500 bg-white text-blue-600 rounded-xl py-3.5 text-center font-semibold text-base mt-2 transition-colors active:bg-blue-50/50 cursor-pointer">
          {{ isExpanded ? '- 접기' : '+ 더보기' }}
        </button>
      </div>
    </div>
