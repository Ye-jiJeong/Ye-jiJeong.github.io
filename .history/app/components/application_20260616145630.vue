...
<template>
  <section class="flex items-end justify-between gap-6 max-md:!items-start max-sm:flex-col max-sm:gap-0">
    ...
    <div class="flex-1 min-w-0 max-sm:w-full max-sm:flex max-sm:flex-col">
      
      <div class="text-base flex gap-2.5 max-sm:order-1 max-sm:mb-4">
      ...
      <h1 class="text-5xl font-bold mt-6.75 mb-5 leading-snug max-md:text-4xl max-sm:text-3xl max-sm:order-3 max-sm:mt-5 max-sm:mb-4">
          {{ titleLine1 }} <br class="max-sm:hidden">
          <span :class="titleClass">{{ titleLine2 }}</span>
          신청
      </h1>

      <ul class="flex flex-col gap-1 max-sm:order-4">
    ...
    <div class="w-fit flex-shrink-0 max-lg:w-[46%] self-start max-sm:w-full max-sm:order-2">
