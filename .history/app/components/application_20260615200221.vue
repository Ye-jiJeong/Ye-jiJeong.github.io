...
<template>
  <section class="flex items-end justify-between max-md:flex-col max-md:items-start max-md:gap-8">
    <div>
      ...
      <ul class="flex flex-col gap-1">
          <li v-for="(item, idx) in checkList" :key="idx" class="text-xl max-lg:text-lg max-md:text-base flex items-center gap-2.5 max-sm:gap-1.5 font-medium">
          <Icon name="lucide:check" :class="titleClass" />
          <span>{{ item.text || item }}</span>
          <Button v-if="item.btnText" :text="item.btnText" class="text-base max-md:text-sm max-sm:text-xs text-primary hover:[&>span]:underline" iconClass="translate-y-[-1px]" />
          </li>
      </ul>
    </div>
  ...
  <section class="mt-16 max-md:mt-10">
    <div class="bg-slate-200 py-7.5 px-8.75 max-md:py-5 max-md:px-5 rounded-2xl">
      ...
      <div class="[&>button]:bg-white [&>button]:py-4 max-md:[&>button]:py-2.5 [&>button]:rounded-xl [&>button]:font-medium text-base max-md:text-sm flex max-md:flex-wrap justify-between gap-2.75 max-md:gap-2 [&>button]:border [&>button]:hover:bg-primary/10 [&>button]:hover:border-primary/10">
