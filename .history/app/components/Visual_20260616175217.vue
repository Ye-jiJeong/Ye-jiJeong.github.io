...
const issueList = ref([
  ...
  {id: 2, theme: 'primary', img: img2, badge: null, label: '빠른 발급', title: '발급기관 직접 방문', desc: ['전국 77개 발급기관 운영', '평일 9~17시 당일 발급'], btnText: '가깝고 편리한 기관 찾기', link: '/sub01_02'},
...
<template>
  <ul class="flex w-full justify-center gap-5 max-lg:flex-col">
    <!-- 교정: 부모 li 영역에서는 공통 테마 식별 클래스만 남기고 배경색 관련 상속 처리를 제거 -->
    <li v-for="issue in issueList" :key="issue.id" :class="['card-item flex-1', issue.theme]">
      
      <!-- 교정: 움직이는 주체인 NuxtLink 컨테이너 자체에 직접 배경색 클래스(bg-theme-mix)를 할당하여 잔상 차단 -->
      <NuxtLink :to="issue.link" class="group relative flex h-full flex-col overflow-hidden rounded-2xl bg-theme-mix card-link max-lg:flex-row max-lg:items-center max-sm:flex-wrap max-lg:p-8 max-lg:p-5 transition-transform duration-300 ease-in-out hover:-translate-y-2 hover:shadow-2xl">
        ...
        <div class="flex grow flex-col items-start py-6 px-7.5 max-lg:py-0 max-lg:pl-8 max-md:pl-4 max-sm:p-0">
          ...
          <h4 class="mt-2 mb-1 md:mb-3.5 font-semibold text-3xl max-lg:text-2xl max-md:text-xl">{{ issue.title }}</h4>
...
.card-item {
  ...
  @each $name, $color in $themes {
    &.#{$name} {
      // 교정: 공용 배경 타겟 명칭(.bg-theme-mix)을 자식 링크 레이어로 이전하여 카드가 공중으로 뜰 때 박스가 겹쳐 보이지 않게 처리
      .bg-theme-mix {
