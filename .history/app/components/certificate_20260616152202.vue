    ...
    <div class="mt-7.5 flex justify-between gap-5 max-lg:flex-col">
      <a :href="plan.link" v-for="plan in certificatePlans" :key="plan.id" :class="['group relative block border-2 rounded-2xl w-full p-7.5 pt-10 bg-white transition-colors', plan.borderColor, plan.hoverBg]">
        ...
        <div class="mt-4.5 mb-6 flex flex-col gap-2">
          <div v-for="(price, pIdx) in plan.priceInfo" :key="pIdx" :class="['flex justify-between w-full min-w-0', price.isFinal ? 'items-baseline text-slate-900/70 mt-2' : 'items-center', price.isDiscount ? plan.textColor : '', price.isOriginal ? 'text-slate-900/30' : '']">
            <span class="flex-shrink-0 text-left">{{ price.label }}</span>
            <span v-if="!price.isFinal" class="flex-shrink-0 text-right">{{ price.value }}</span>
            
            <div v-else class="flex items-end flex-shrink-0 text-right whitespace-nowrap pl-4 ml-auto">
              <span class="text-slate-900/30 text-sm mr-1.25 mb-1.5 max-sm:mb-0">{{ price.subLabel }}</span>
              <strong class="text-black text-4xl max-sm:text-3xl leading-none">{{ price.value }}</strong>
            </div>
          </div>
        </div>

        <Button :text="plan.btnText" hideArrow :class="['text-white w-full py-5 text-xl font-medium mt-7.5 max-md:text-lg', plan.bgColor]" />
      </a>
    </div>
  </div>
