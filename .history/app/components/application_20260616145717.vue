...
<template>
  <section class="flex items-end justify-between gap-6 max-md:!items-start max-sm:flex-col max-sm:gap-0">
    <div class="flex-1 min-w-0 max-sm:w-full">
      ...
      <div class="hidden max-sm:block w-full my-4">
        <div class="relative inline-block w-full">
          ...
          <p v-if="visualBadgeText" class="absolute top-2.5 right-2.5 inline-block rounded bg-warning py-0.5 px-2 font-bold text-xs whitespace-nowrap">
              {{ visualBadgeText }}
          </p>
        </div>
      </div>
    ...
    <div class="w-fit flex-shrink-0 max-lg:w-[46%] self-start max-sm:hidden">
      <div class="relative inline-block w-full">
