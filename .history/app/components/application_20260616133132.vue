...
<template>
  <section class="flex items-end justify-between max-md:flex-col max-md:items-start max-md:gap-8 ma">
    <div>
      ...
      <h1 class="text-5xl font-bold mt-6.75 mb-5 leading-snug max-md:text-4xl max-sm:text-3xl">
          {{ titleLine1 }} <br class="max-md:hidden">
          <span :class="titleClass">{{ titleLine2 }}</span>
          신청
      </h1>

      <ul class="flex flex-col gap-1">
          <li v-for="(item, idx) in checkList" :key="idx" class="text-xl flex items-center gap-2.5 font-medium max-sm:flex-wrap max-xl:flex-wrap">
    ...
    <div class="w-fit max-md:w-full max-md:flex max-md:justify-center max-md:mt-6">
      <div class="relative inline-block max-md:w-full">
        <img :src="imgUrl" alt="비주얼 이미지" class="" />
  ...
  <section class="mt-16">
    <div class="bg-slate-200 py-7.5 px-8.75 rounded-2xl">
      ...
      <div class="text-base flex flex-wrap justify-start min-xl:flex-nowrap min-xl:justify-between gap-2.75">
        <button v-for="(tab, idx) in useList" :key="idx" type="button" :class="['bg-white py-4 rounded-xl font-medium border border-white pointer-events-none', tabClass]">{{ tab }}</button>
        <button type="button" class="bg-white py-4 rounded-xl font-medium border border-primary !px-5.5 text-primary hover:bg-primary/10 hover:border-primary/10 transition-colors">+ 더보기</button>
      </div>
    </div>
  </section>
