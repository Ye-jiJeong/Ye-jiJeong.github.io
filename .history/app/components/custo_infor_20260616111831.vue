...
const formSections = ref([
  ...
  {
    id: 'interest',
    label: '관심<br class="max-[440px]:hidden">정보',
    type: 'checkbox',
    wrapperClass: 'flex flex-wrap gap-x-4 gap-y-[11px]',
    options: [
      { id: 'tax', label: '세무·금융' },
...
<template>
  <div class="w-full mx-auto flex flex-col items-center px-11.25 max-sm:px-8 mobile-px-0">
    ...
    <div class="w-full bg-muted-light border border-bs-gray rounded-2xl p-6 pb-8.5 flex flex-col gap-6 my-7.5 max-sm:my-5">
      <div v-for="section in formSections" :key="section.id" class="flex items-start mobile-stack">
        ...
        <div :class="section.wrapperClass" class="max-sm:flex max-sm:flex-wrap">
          <template v-for="(item, index) in section.options" :key="item.id">
            <label class="flex items-center text-base text-[#6C757D] cursor-pointer">
              <input :type="section.type" :name="section.id" :value="item.id" v-model="formModels[section.id]" class="appearance-none w-5 h-5 rounded-full border border-[#CED4DA] bg-white checked:border-primary checked:bg-primary mr-2 max-sm:mr-1.5 max-sm:w-4 max-sm:h-4">
              {{ item.label }}
            </label>
          </template>
        </div>
...
/* 440px 이하 모바일 화면을 위한 미디어 쿼리 (우선순위 강제 적용) */
@media (max-width: 440px) {
  ...
  .mobile-stack {
    ...
    gap: 0.5rem !important; /* Tailwind의 gap-2와 동일 (8px) */
  }
}
</style>