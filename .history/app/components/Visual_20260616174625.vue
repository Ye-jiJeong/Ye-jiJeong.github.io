...
<template>
  <ul class="flex w-full justify-center gap-5 max-lg:flex-col">
    <li v-for="issue in issueList" :key="issue.id" :class="['card-item rounded-2xl flex-1', issue.theme]">
      ...
      <NuxtLink :to="issue.link" class="group relative flex h-full flex-col overflow-hidden rounded-2xl card-link max-lg:flex-row max-lg:items-center max-sm:flex-wrap max-lg:p-8 max-lg:p-5 transition-transform duration-300 ease-in-out hover:-translate-y-2">
