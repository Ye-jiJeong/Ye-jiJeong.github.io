...
<template>
  <section class="flex items-end justify-between gap-6 max-md:!items-start max-sm:flex-col-reverse">
    <div class="flex-1 min-w-0">
      ...
      <ul class="flex flex-col gap-1">
          <li v-for="(item, idx) in checkList" :key="idx" class="text-xl flex items-center gap-2.5 font-medium whitespace-nowrap max-md:text-base">
            <Icon name="lucide:check" :class="[titleClass, 'flex-shrink-0']" />
    ...
    <div class="w-fit flex-shrink-0 max-lg:w-[46%] self-start">
      <div class="relative inline-block w-full">
        <img :src="imgUrl" alt="비주얼 이미지" class="w-full" />
        <p v-if="visualBadgeText" class="absolute top-6 right-5 inline-block rounded-lg bg-warning py-1 px-4 font-bold text-lg whitespace-nowrap max-md:rounded max-md:top-3 max-md:right-3 max-md:py-0.5 max-md:px-2 max-md:text-base">
            {{ visualBadgeText }}
        </p>
      </div>
    </div>
