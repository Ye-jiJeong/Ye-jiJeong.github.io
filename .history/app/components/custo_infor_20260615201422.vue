...
<template>
  <div class="w-full mx-auto flex flex-col items-center px-11.25">
    ...
    <p class="text-center text-black/70 text-lg">맞춤 소식을 전달드리기 위해<br>간단한 정보를 선택적으로 받고 있습니다.</p>

    <div class="w-full bg-muted-light border border-bs-gray rounded-2xl p-6 pb-8.5 flex flex-col gap-6 my-7.5">
      <div v-for="section in formSections" :key="section.id" class="flex items-start">
        <span class="mr-4 text-lg font-medium text-black flex-shrink-0" v-html="section.label"></span>
        <div :class="section.wrapperClass" class="">
          <template v-for="(item, index) in section.options" :key="item.id">
            <label class="flex items-center text-bace text-[#6C757D] cursor-pointer">
              <input :type="section.type" :name="section.id" :value="item.id" v-model="formModels[section.id]" class="appearance-none w-5 h-5 rounded-full border border-[#CED4DA] bg-white checked:border-primary checked:bg-primary mr-2">
              {{ item.label }}
            </label>
          </template>
        </div>
