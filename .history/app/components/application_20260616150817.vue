  ...
  <section class="flex items-end justify-between gap-6 max-md:!items-start max-sm:flex-col max-sm:gap-0">
    ...
    <div class="flex-1 min-w-0 max-sm:w-full">
      ...
      <div class="hidden max-sm:block w-full my-4">
        <div class="relative inline-block w-full">
          ...
          <p v-if="visualBadgeText" class="absolute max-sm:!top-8 max-sm:!right-8 inline-block rounded bg-warning py-0.5 px-2 font-bold text-base whitespace-nowrap">
              {{ visualBadgeText }}
          </p>
        </div>
      </div>
    ...
    <div class="w-fit flex-shrink-0 max-lg:w-[46%] max-sm:hidden">
      <div class="relative inline-block w-full">
        ...
        <p v-if="visualBadgeText" class="absolute top-6 right-5 inline-block rounded-lg bg-warning py-1 px-4 font-bold text-lg whitespace-nowrap max-md:rounded max-md:top-3 max-md:right-3 max-md:text-base max-sm:top-4 max-sm:right-4">
            {{ visualBadgeText }}
        </p>
      </div>
    </div>
