...
const filterCategories = ref([
  { id: 'period', selected: '', defaultText: '인증서 기간', options: ['1년형', '3년형'], widthClass: 'w-[170px]' },
  { id: 'bizType', selected: '', defaultText: '사업자 유형', options: ['개인사업자', '법인사업자'], widthClass: 'w-[170px]' },
  { id: 'applicant', selected: '', defaultText: '신청자', options: ['대표', '대리인'], widthClass: 'w-[130px]' }
]);
...
<template>
  <div class="w-full">
    ...
    <div class="px-20 py-16.25 border border-slate-100 rounded-2xl flex items-center justify-between gap-25">
      <div class="text-center ml-5">
        <p class="text-xl text-slate-900">고객 만족도</p>
        <TitleH2 text="4.8" class="my-2.25" />
        <p class="text-base text-slate-400">(1,234건 후기)</p>
        
        <div class="flex justify-center gap-0.05 mt-4 text-warning text-3xl">
      ...
      <div class="w-full">
        <p class="text-slate-500">1,234명의 실제 고객님들의 후기 입니다.</p>
    ...
    <div class="mt-13.5 flex items-center justify-between border-slate-100 mb-4">
      <div class="flex gap-2 mb-7.5">
        <div v-for="category in filterCategories" :key="category.id" class="relative">
          <select 
            v-model="category.selected" 
            :class="['appearance-none border border-slate-200 rounded-lg py-2 px-4 text-base text-slate-700 cursor-pointer focus:outline-none focus:border-blue-500', category.widthClass]"
          >
            <option value="">{{ category.defaultText }}</option>
            <option v-for="(opt, idx) in category.options" :key="idx" :value="opt">{{ opt }}</option>
          </select>
          
          <icon name="bxs:down-arrow" class="absolute right-4.5 top-1/2 -translate-y-1/2 text-xs text-slate-500" />
        </div>
      </div>

      <div class="flex items-center text-base">
        <button class="text-primary" >최신순</button>
        <button class="text-slate-900 before:content-['|'] before:text-slate-200 before:px-4">평점순</button>
      </div>
    </div>
    ...
    <div class="flex flex-col gap-7.5">
      <div 
        ...
        :key="review.id" 
        class="py-8 px-6 flex items-start justify-between border-b border-slate-200 rounded-3xl"
      >
        <div class="flex items-center gap-4">
          <img :src="reviewImages[index % 4]" alt="고객 프로필" class="" />
          ...
          <div>
            ...
            <p class="text-lg mt-2">{{ review.content }}</p>
          </div>
        </div>
        
        <div class="text-right text-sm text-slate-700/40 whitespace-nowrap pt-1">
          <span class="text-slate-500">{{ review.type }}</span>
          <span class="before:content-['|'] before:text-slate-200 before:px-2 after:content-['|'] after:text-slate-200 after:px-2">{{ review.name }}</span>
