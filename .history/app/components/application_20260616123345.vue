...
<template>
  ...
  <section class="mt-16">
    <div class="bg-slate-200 py-7.5 px-8.75 rounded-2xl">
      ...
      <div class="[&>button]:bg-white [&>button]:py-4 [&>button]:rounded-xl [&>button]:font-medium text-base flex justify-between gap-2.75 max-lg:flex-wrap">
        <button v-for="(tab, idx) in useList" :key="idx" type="button" :class="['border-white', tabClass]">{{ tab }}</button>
        <button type="button" class="!px-5.5 border-primary  [&>button]:border [&>button]:hover:bg-primary/10 [&>button]:hover:border-primary/10 ">+ 더보기</button>
      </div>
    </div>
  </section>
