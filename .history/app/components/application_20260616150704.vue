  ...
  <section class="flex items-end justify-between gap-6 max-md:!items-start max-sm:flex-col max-sm:gap-0">
    ...
    <div class="w-fit flex-shrink-0 max-lg:w-[46%] max-sm:hidden">
      <div class="relative inline-block w-full">
        ...
        <p v-if="visualBadgeText" class="absolute top-6 right-5 inline-block rounded-lg bg-warning py-1 px-4 font-bold text-lg whitespace-nowrap max-md:rounded max-md:top-3 max-md:right-3 max-md:text-base max-sm:top-5 max-sm:right-5">
            {{ visualBadgeText }}
        </p>
      </div>
    </div>
