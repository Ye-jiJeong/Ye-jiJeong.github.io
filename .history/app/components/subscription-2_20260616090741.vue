...
const selectedTab = ref('subscribe')
const tabs = ref([{ id: 'subscribe', label: '구독신청' }, { id: 'unsubscribe', label: '구독취소' }])
...
<template>
  <div class="w-full flex flex-col items-center">
    ...
    <div class="w-full max-w-128 bg-muted-light border border-bs-gray rounded-2xl p-6 flex flex-col gap-2">
      <div v-for="field in inputFields" :key="field.id" class="flex gap-2">
        <input :id="field.id" :type="field.type" :placeholder="field.placeholder" autocomplete="username" aria-required="true" class="flex-1 h-12 px-4 bg-white border border-gray-300 rounded-lg text-base focus:outline-none focus:border-primary">
        <div v-if="field.wrapperClass" :class="field.wrapperClass">
          <button type="button" @click="field.id === 'auth-input' ? $emit('nextStep') : null" :class="field.btnClass">{{ field.btnText }}</button>
        </div>
        <button v-else type="button" @click="field.id === 'auth-input' ? $emit('nextStep') : null" :class="field.btnClass">{{ field.btnText }}</button>
      </div>
